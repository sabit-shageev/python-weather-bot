...
from config import WEATHER_API_KEY, WEATHER_URL, FORECAST_URL
...
def get_weather(city):
    ...
    if cached:
        logger.debug(f"📦 Кэш для {city}")
        return cached

    logger.debug(f"🌍 Запрос в API для {city}")

    # 2. Запрос к API
    url = f"{WEATHER_URL}?q={city}&appid={WEATHER_API_KEY}&units=metric&lang=ru"
    try:
...
def get_weather_forecast(city):
    ...
    url = f"{FORECAST_URL}?q={city}&appid={WEATHER_API_KEY}&units=metric&lang=ru"
    try:
        data = requests.get(url, timeout=5).json()
    except Exception as e:
...
def get_timezone_offset(city: str) -> int:
    ...
    """
    try:
        url = f"{FORECAST_URL}?q={city}&appid={WEATHER_API_KEY}&units=metric"
